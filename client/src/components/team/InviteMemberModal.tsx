...
export default function InviteMemberModal({
  ...
  const [inviteMethod, setInviteMethod] = useState<"existing" | "email">(
    "existing"
  );
    ...
    const loadBoards = async () => {
      ...
      try {
        ...
        const boards = response.data.boards.filter(
          (board: any) => board.owner.auth0Id == user?.sub
        );
  ...
  }, [searchQuery, existingMembers, dbUser, inviteMethod]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (inviteMethod === "existing" && !selectedUser) {
    ...
    try {
      const selectedBoardData = userBoards.find((b) => b.id === selectedBoard);

      const inviteData = {
        boardId: selectedBoard,
        boardTitle: selectedBoardData?.title,
        userId: selectedUser?.id,
        email: inviteMethod === "email" ? inviteEmail : selectedUser?.email,
        name:
          inviteMethod === "email"
            ? inviteEmail.split("@")[0]
            : selectedUser?.name,
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                type="button"
                onClick={() => toast.info("Email invites are coming soon!")}
                className={cn(
                  "flex items-center justify-center p-3 border-2 rounded-lg transition-all",
                  inviteMethod === "email"
                    ? "border-primary-500 bg-primary-50 dark:bg-primary-900/20"
            ...
            <div className="relative">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  required={inviteMethod === "existing"}
          ...
          {selectedUser && inviteMethod === "existing" && (
          ...
          {/* Invitation Preview */}
          {(selectedUser || inviteEmail) && selectedBoard && (
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
              ...
              <div className="text-sm text-blue-800 dark:text-blue-200">
                <p>
                  ...
                  <strong>
                    {userBoards.find((b) => b.id === selectedBoard)?.title}
                  </strong>{" "}
                  as a <strong>{role.toLowerCase()}</strong>.
                </p>
                <p className="mt-2 text-xs">
                  They will receive an email invitation with instructions to
                  join the board.
                </p>
              </div>
            </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <Button
              ...
              disabled={
                (inviteMethod === "existing" && !selectedUser) ||
